...
- *public* - извне видно всем
- *protected* - извне видно только дочерним/наследуемым/производным классам (это одно и то же)
- *private* - извне никому не видно
...
- protected - данные видны только как protected
- private - все унаследованные данные становится private
...
public: 
    ...
    const int getParam() const //getter
        { return _a; }
...
int main() {
   const int i = 5; // грубо говоря, i теперь read-only
   i = 10;   // error
   i++;   // error
}
```
