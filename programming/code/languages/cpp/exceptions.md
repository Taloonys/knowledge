...
# multiple exceptions
> По документации С++ выбрасывание нескольких исключений одновременно приводит к вызову `std::terminate`.
...
}                                // второе выбрасывание исключения в том же контексте
/* вызов std::terminate() от RTI языка
catch(...) {
	// ...
}
```
