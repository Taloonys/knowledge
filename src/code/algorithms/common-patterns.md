...
## Подходы
### Мемоизация
>... начинаем с основной задачи, рекурсивно вызывая подзадачи, каждое решение подзадачи сохраняем в памяти, если подзадача встречается повторно, ты мы просто используем её результат 
* (*см пример с tail recursion*)
### Табуляция
> ... сначала составляем "таблицу" с результатами всех подзадач, затем используем подзадачи для решения основной задачи.
* это как если бы в *tail recursion* мы заменили рекурсию на что-то вроде:
  `dp = [0, 1] ... for i: dp.append(dp[i - 1] + dp[i - 2])`
## Most common cases
* fibonacci numbers
* kadane's algorithm
* 0/1 knapsack 
