...
> Фабричный метод — базовый класс знает, что надо создать, но делегирует конкретный тип потомкам.
...
class AnimalFactory:
    def make(self, kind):
        return Cat() if kind == "cat" else Dog()

pet = AnimalFactory().make("cat")  # client видит только Animal
```
